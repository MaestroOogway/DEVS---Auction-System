...
# ===========  B) Extraer Productos Comprados ===========

N_PRODUCTOS = 10  # número real de productos
...
for ruta in archivos:
    ...
    with open(ruta, "r", encoding="utf-8") as f:
        for linea in f:
            ...
            # Capturar el modelo y el IdAgent
            ...
            # Sobrescribir último vector para este agente
            if tipo == "affective":
                latest_afectives[agent_id] = vector
            else:  # rational
                latest_racionals[agent_id] = vector

    # Comprobar que al menos un agente de cada tipo tenga un vector final
    if not latest_afectives or not latest_racionals:
        print(f"faltan agentes en {os.path.basename(ruta)}")
        continue

    # Sumar elemento a elemento todos los vectores finales de los agentes de cada tipo
